...
var FIELD = {
  ...
  PEOPLE: { id:3, name:7, email:8, role:11, active:19, podName:22, tdId:23, partTime:24, hourlyRate:43, qbRoleId:44, qbUserId:45 },
...
async function loadPeople(activeOnly=true) {
  ...
  const rows = await qbQueryAll(TABLES.people,
    [FIELD.PEOPLE.id, FIELD.PEOPLE.name, FIELD.PEOPLE.email, FIELD.PEOPLE.role,
     FIELD.PEOPLE.active, FIELD.PEOPLE.podName, FIELD.PEOPLE.tdId, FIELD.PEOPLE.partTime, FIELD.PEOPLE.hourlyRate, FIELD.PEOPLE.qbRoleId, FIELD.PEOPLE.qbUserId],
    where);
  return rows.map(r => ({
    ...
    qbUserId: fv(r, FIELD.PEOPLE.qbUserId) || ''
